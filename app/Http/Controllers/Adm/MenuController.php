...
class MenuController extends Controller
{
    ...
    {
        $menu = Menu::with('children.children.children.children.children')->where('parent_id', 0)->orderBy('order', 'asc')->get();
        $role = Role::with('menus')->find($request->role);
        $menus = array();
        if ($role && count($role->menus) > 0){
            foreach($role->menus as $val){
                $menus[] = $val->id;
            }
        }
        $menuTree = $this->buildMenuTree($menu->toArray(), $menus);
    ...
    public function buildMenuTree($menu, $menus){ 
        $result = array();
                ...
                ".(($item['id'] > 1) ? "<div class='dd-handle dd3-handle'></div>":"")."
                    <div class='dd3-content'>
                        ...
                        <div class='pull-right'>
                            ...
                            ".(($item['id'] > 1) ? "| <form action='/adm/menu/{$item['id']}' method='POST' class='pull-right'>
										".Form::token()."
    ...
    {
        foreach($data as $val){
            Menu::where('id', $val['id'])->update(['parent_id' => $parent, 'order' => $order]);
            $order++;
