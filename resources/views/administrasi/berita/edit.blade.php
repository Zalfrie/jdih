        ...
        <div class="col-md-12">
			<div class="portlet box blue-chambray">
				<div class="portlet-title">
					<div class="caption">
						<i class="icon-book-open"></i> Edit Berita
					</div>
				</div>
				<div class="portlet-body form">
                    @include('common.errors')
					<form id="ubahDataForm" class="form-horizontal" action="/administrasi/berita/{{ $data->berita_id }}" method="POST" role="form">
                    {{ csrf_field() }}
                        {{ method_field('PUT') }}
						<div class="form-body">
                            <div class="row">
                                <div class="col-md-9">
        							<div class="form-group">
                                        ...
                                        <div class="col-md-10">
        								    <input type="text" name="judul" class="form-control input-sm" value="{{ $data->title }}" >
                                        </div>
        							</div>
        							<div class="form-group">
        								<label class="col-md-2 control-label">Konten</label>
                                        <div class="col-md-10">
        								    <textarea class=" form-control" id="berita-editor" rows="6" name="konten" >{{ nl2br($data->content) }}</textarea>
                                        </div>
        							</div>
        							<div class="form-group">
        							...
        							<div class="form-group">
                                        ...
                                        <div class="col-md-10">
        									<div class="icheck-inline">
        										<label>
        										<input type="radio" name="status" value="draft" {{(!$data->is_publish)? 'checked':''}} class="icheck" data-radio="iradio_square-grey"> Draft </label>
        										<label>
        										<input type="radio" name="status" value="publish" {{($data->is_publish)? 'checked':''}} class="icheck" data-radio="iradio_square-grey"> Publish </label>
        									</div>
                                        </div>
        							</div>
                                </div>
                                <div class="col-md-3">
        							<div class="form-group">
                                        ...
                                        <div class="input-group fix-margin">
                                            ...
                                            <?php
                                            $selectedCat = $data->kategori->pluck('kategori_id')->all();
                                            ?>
                                                @if (count($kategori) > 0)
                                                @foreach ($kategori as $val)
												<label>
												<input type="checkbox" name="kategori[]" {{in_array($val->kategori_id, $selectedCat)?'checked':''}} value="{{$val->kategori_id}}" class="icheck" data-checkbox="icheckbox_square-grey"> {{$val->kategori}} </label>
                                                @endforeach
                                                @endif
											</div>
